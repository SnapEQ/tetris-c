						...
						enum color c)
{
	...
	filledPolygonRGBA(renderer, xtab, ytab, 3, colors[c].r, colors[c].g,
					  colors[c].b, 255);
}
...
{
	assert(c < COLOR_MAX);
	rectangleRGBA(renderer, x1, y1, x2, y2, colors[c].r, colors[c].g,
				  colors[c].b, 255);
}
...
{
	assert(c < COLOR_MAX);
	boxRGBA(renderer, x1, y1, x2, y2, colors[c].r, colors[c].g, colors[c].b,
			255);
}
